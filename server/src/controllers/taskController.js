...
const validateAssignee = async (assigneeId, project) => {
  if (!project || !project.workspaceId) throw new HttpError(400, 'Invalid project context');
...
export const listTasks = asyncHandler(async (req, res) => {
  if (!req.project) throw new HttpError(404, 'Project not found');
...
export const createTask = asyncHandler(async (req, res) => {
  ...
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers have read-only access and cannot create tasks');
  }
...
export const updateTask = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers have read-only access and cannot modify tasks');
  }
...
export const reorderTask = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot reorder tasks');
  }
...
export const bulkUpdateTasks = asyncHandler(async (req, res) => {
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot perform bulk task updates');
  }
  ...
  if (!operations.length) return res.json({ success: true, modifiedCount: 0 });
...
export const deleteTask = asyncHandler(async (req, res) => {
  ...
  if (!['Owner', 'Admin'].includes(userRole)) {
    throw new HttpError(403, 'Only Owners and Admins can delete tasks');
  }
...
export const addSubtask = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot add subtasks');
  }
...
export const updateSubtask = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot update subtasks');
  }
...
export const deleteSubtask = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot delete subtasks');
  }
...
export const addComment = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers cannot add comments');
  }
