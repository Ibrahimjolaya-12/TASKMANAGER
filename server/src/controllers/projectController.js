...
export const createProject = asyncHandler(async (req, res) => {
  ...
  if (userRole === 'Viewer') {
    throw new HttpError(403, 'Viewers are not allowed to create projects');
  }
...
export const listProjects = asyncHandler(async (req, res) => {
  ...
  const counts = await Task.aggregate([
    { $match: { workspaceId: new mongoose.Types.ObjectId(req.workspace._id) } },
    { 
      $group: { 
        _id: '$projectId', 
        total: { $sum: 1 }, 
...
export const updateProject = asyncHandler(async (req, res) => {
  ...
  if (!['Owner', 'Admin'].includes(userRole)) {
    throw new HttpError(403, 'Only Owners and Admins can update projects');
  }
...
export const deleteProject = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (userRole !== 'Owner') {
    throw new HttpError(403, 'Only the Workspace Owner can delete projects');
  }
...
export const addProjectMember = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (!['Owner', 'Admin'].includes(userRole)) {
    throw new HttpError(403, 'Not authorized to add project members');
  }
  ...
  res.status(201).json({ success: true, member: { userId, role, workspaceRole: wsMember.role } });
});
...
export const removeProjectMember = asyncHandler(async (req, res) => {
  ...
  const userRole = req.effectiveRole || 'Viewer';
  if (!['Owner', 'Admin'].includes(userRole)) {
    throw new HttpError(403, 'Not authorized to remove project members');
  }
