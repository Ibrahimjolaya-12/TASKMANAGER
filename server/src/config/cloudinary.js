...
/** Upload a multer memory buffer safely with stream error catching. */
  ...
  new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      ...
      (err, result) => {
        ...
        if (!result) return reject(new Error('Cloudinary upload returned an empty response.'));
        resolve({
...
export const destroyAsset = (publicId) => 
  cloudinary.uploader.destroy(publicId, { resource_type: 'raw' })
    ...
    .catch(() => cloudinary.uploader.destroy(publicId));