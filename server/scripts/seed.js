/**
 * Seed script: wipes database and creates a demo workspace with dynamic roles.
...
const seed = async () => {
  ...
  const userData = [
    { name: 'Muhammad ibrahim', email: 'ibrahimjolaya2@gmail.com', role: 'Owner' },
  ...
  // Create Workspace dynamically using the Owner reference
  const workspace = await Workspace.create({
    name: 'Acme Inc',
    slug: 'acme-inc',
    ownerId: byEmail.Owner._id,
    members: userData.map((u) => ({
      userId: byEmail[u.role]._id,
      role: u.role,
    })),
  });

  // Assign workspace references and dynamic roles to users
  for (const u of userData) {
    await User.updateOne(
      { _id: byEmail[u.role]._id },
      {
        $set: {
          workspaces: [{ workspaceId: workspace._id, role: u.role }],
