...
import { Modal, Form, Input, ColorPicker, Button, Alert } from 'antd';
...
export default function AppLayout() {
  ...
  const handleCreateProject = (values) => {
    const hexColor = typeof values.color === 'string' ? values.color : values.color?.toHexString?.() || '#6366f1';
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="Project name" size="large" autoFocus />
          </Form.Item>
          ...
          <Form.Item
            name="color"
            label={<span className="text-xs font-medium text-slate-600">Color</span>}
          >
          ...
          {createProject.error && (
            <div className="mb-4">
              <Alert message={apiError(createProject.error)} type="error" showIcon />
            </div>
          )}

          <div className="flex justify-end gap-2 pt-2">
            <Button onClick={() => setProjectOpen(false)}>
              Cancel
            </Button>
            <Button
              type="primary"
